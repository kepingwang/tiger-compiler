...
fun codegen frame stm =
  let
      ...
      fun munchStm (T.LABEL lab) = emit(
        ...
        | munchStm (T.CJUMP (T.NE, exp1, exp2, t_label, f_label)) = emit(A.OPER{
                                                                              assem="bne `s0, `s1, "^(Symbol.name t_label),
                                                                              src=[munchExp exp1, munchExp exp2],
                                                                              dst=[],
                                                                              jump = SOME [t_label, f_label]
                                                                        })
        | munchStm (T.CJUMP (T.LT, exp1, exp2, t_label, f_label)) = emit(A.OPER{
                                                                              assem="blt `s0, `s1, "^(Symbol.name t_label),
                                                                              src=[munchExp exp1, munchExp exp2],
                                                                              dst=[],
                                                                              jump = SOME [t_label, f_label]
                                                                        })
        | munchStm (T.CJUMP (T.GT, exp1, exp2, t_label, f_label)) = emit(A.OPER{
                                                                              assem="bgt `s0, `s1, "^(Symbol.name t_label),
                                                                              src=[munchExp exp1, munchExp exp2],
                                                                              dst=[],
                                                                              jump = SOME [t_label, f_label]
                                                                        })
        | munchStm (T.CJUMP (T.LE, exp1, exp2, t_label, f_label)) = emit(A.OPER{
                                                                              assem="ble `s0, `s1, "^(Symbol.name t_label),
                                                                              src=[munchExp exp1, munchExp exp2],
                                                                              dst=[],
                                                                              jump = SOME [t_label, f_label]
                                                                        })
        | munchStm (T.CJUMP (T.GE, exp1, exp2, t_label, f_label)) = emit(A.OPER{
        ...
        | munchStm (e) = (print("uncatched stmt:\n"); Printtree.printtree(TextIO.stdOut, e); raise Unmatched)
      and result(gen) =
          let
              val t = Temp.newtemp()
          in
      ...
      and munchExp (T.BINOP (T.PLUS, T.TEMP src1, T.TEMP src2)) = result(fn (r)=>
        ...
        | munchExp (T.CALL(T.NAME func_label, params)) =
          result ( fn (r) => (
                       emit(A.OPER{
                                 assem="sub $sp, $sp, " ^ (Int.toString (Frame.wordSize * (List.length params))),
                                 src=[Frame.SP],
                                 dst=[Frame.SP],
                                 jump=NONE
                           });
                       ...
                       emit(A.OPER{
                                 assem="add $sp, $sp, " ^ (Int.toString (Frame.wordSize * (List.length params))),
                                 src=[Frame.SP],
                                 dst=[Frame.SP],
                                 jump=NONE
                           });
        ...
        | munchExp (T.NAME lab) = result ( fn(r) => emit(
                                                       A.OPER({
        ...
        | munchExp (e) = (print("uncatched exp:\n"); Printtree.printtree(TextIO.stdOut, T.EXP(e)); raise Unmatched)
  in
      munchStm stm;
      rev (!assem_list)
  end
